...
function createPane(opts = {}) {
  ...
  // Spawn the shell after the pane is in the DOM so fitAddon.fit() has real
  // dimensions. Without the RAF, the shell spawns at 80×24 (fallback) then
  // immediately gets a SIGWINCH from the post-relayout resize, which causes
  // zsh's ZLE to lose the initial prompt render.
