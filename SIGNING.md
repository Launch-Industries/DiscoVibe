...
`notarize.teamId` = `K2W6NQ6F66`). It needs two things only your Apple account can
...
1. Open **Xcode → Settings → Accounts**, sign in with the Apple ID on the Developer
   Program (Team `K2W6NQ6F66`).
2. Select the team → **Manage Certificates… → + → Developer ID Application**.
3. It installs into your login keychain. Verify:
   ```
   security find-identity -v -p codesigning   # should list: Developer ID Application: … (K2W6NQ6F66)
   ```
...
## Notes
- The membership fee is Apple's ($99/yr); the Team ID `K2W6NQ6F66` is already wired in.
